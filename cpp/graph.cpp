...
void Graph::runSelectedSearch(const std::string& searchType, const std::string& stat) {
    ...
    std::vector<std::pair<std::string, double>> BFSresult;
    ...
    if (searchType == "S") {
        ...
        auto BFSresult = bfs(startNode);
        exportResults(BFSresult, "bfs", stat);
        exportAndPrepareForViewer(*this, BFSresult, "bfs", stat, startNode);        
